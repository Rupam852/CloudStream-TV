...
serve(async (req) => {
  ...
  try {
    ...
    if (path === '/login' || path === '/api/login') {
      ...
      if (!sessionId) {
        return Response.redirect(`https://cloudstream-tv.vercel.app/success?status=error&message=Missing%20session%20parameter%20in%20the%20URL.%20Please%20scan%20the%20QR%20code%20or%20enter%20the%20code%20manually.&t=${Date.now()}`, 302);
      }
      ...
      if (error || !session) {
        return Response.redirect(`https://cloudstream-tv.vercel.app/success?status=error&message=This%20login%20session%20is%20invalid%20or%20has%20expired.%20Please%20close%20the%20login%20screen%20on%20your%20TV%20and%20open%20it%20again.&t=${Date.now()}`, 302);
      }
    ...
    if (path === '/callback' || path === '/api/callback') {
      ...
      if (!code || !stateStr) {
        return Response.redirect(`https://cloudstream-tv.vercel.app/success?status=error&message=Missing%20authorization%20code%20or%20state%20from%20Google.%20Please%20try%20logging%20in%20again.&t=${Date.now()}`, 302);
      }
      ...
      if (error || !session) {
        return Response.redirect(`https://cloudstream-tv.vercel.app/success?status=error&message=This%20login%20session%20has%20expired%20or%20is%20invalid.%20Please%20close%20the%20login%20screen%20on%20your%20TV%20and%20open%20it%20again.&t=${Date.now()}`, 302);
      }
      ...
      return Response.redirect(`https://cloudstream-tv.vercel.app/success?status=success&name=${encodedName}&t=${Date.now()}`, 302);
    }
